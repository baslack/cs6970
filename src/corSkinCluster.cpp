...
const MString corSkinCluster::kname("corSkinCluster");

const MTypeId corSkinCluster::id(0x22573);

const double corSkinCluster::omega(DEFAULT_OMEGA);

const int corSkinCluster::_profileCategory(
    MProfiler::addCategory(NAME));
...
MStatus corSkinCluster::initialize()
{
    ...
    cor_valid =
        numeric_fn.create("Valid_Precomputation", "valid",
    ...
    cor_ar = typed_fn.create("Centers_of_Rotation",
            "cor", MFnData::Type::kPointArray, default_ar_obj,
            &status);
    if(status != MS::kSuccess)
        {
...
MStatus corSkinCluster::handle_to_doublearray(
    MArrayDataHandle& handle, MDoubleArray& vec)
{
    int count = 0;
...
MStatus corSkinCluster::similarity(
    MDoubleArray& weight_p,
...
MStatus corSkinCluster::qlerp(MQuaternion& q_a,
    MQuaternion& q_b, MQuaternion& result)
{
    MStatus stat;
    double dot_product;
    double q_a_comp[4];
    ...
    if(stat != MStatus::kSuccess)
        {
            std::cerr <<
                "corSkinCluster::qlerp, unable to extract q_a" <<
                std::endl;
    ...
    if(stat != MStatus::kSuccess)
        {
            std::cerr <<
                "corSkinCluster::qlerp, unable to extract q_b" <<
                std::endl;
            return MStatus::kFailure;
        }
    dot_product = 0.0;
...
MStatus corSkinCluster::precomp(MDataBlock block)
{
    ...
    MDataHandle cor_arHandle = block.inputValue(
            cor_ar);
    ...
    cor_PA.clear();
    // get mesh iterator
    MArrayDataHandle inputHandle =
        block.inputArrayValue(input);
    stat = inputHandle.jumpToArrayElement(0);
    if(stat != MS::kSuccess)
        return stat;
    MDataHandle cor_IOGeoHandle =
        inputHandle.inputValue().child(inputGeom);
    ...
    // weights
    MArrayDataHandle w_i = block.inputArrayValue(
            weightList);
    ...
    // bone transforms
    MArrayDataHandle transformHandle =
        block.inputArrayValue(matrix);
    int num_transforms =
        transformHandle.elementCount();
            ...
            if(stat == MStatus::kSuccess)
                {
                    ...
                    for(idx = 0; idx < num_tris; idx++)
                        {
                            // get the verts
                            stat = T.getTriangle(idx, tri_verts, tri_idx,
                                    MSpace::kObject);  // switched this to world, from kObject
                            if(stat.error())
                                {
                            ...
                            stat = tri_area.append(((beta_alpha ^
                                            gamma_alpha).length()) * 0.5);
                            if(stat.error())
                                {
                            ...
                            stat = tri_avg_pos.append((alpha + beta + gamma) /
                                    3);
                            if(stat.error())
                                {
                            ...
                            MArrayDataHandle alpha_weightsHandle =
                                w_i.inputValue().child(weights);
                            // get beta weights
                            ...
                            MArrayDataHandle beta_weightsHandle =
                                w_i.inputValue().child(weights);
                            // get gamma weights
                            ...
                            MArrayDataHandle gamma_weightsHandle =
                                w_i.inputValue().child(weights);
                            double a, b, c;
    ...
    while(!(v_i.isDone()))
        {
            ...
            MArrayDataHandle vertex_weights_handle =
                w_i.inputValue().child(weights);
            for(int i = 0; i < num_transforms; i++)
                {
                    ...
                    if(stat.error())
                        ...
                        vertex_weights.append(
                            vertex_weights_handle.inputValue().asDouble());
                }
            s = 0.0;
            ...
            for(int i = 0; i < num_tris; i++)
                {
                    stat = similarity(vertex_weights,
                            tri_avg_weights[i], num_transforms, s);
...
//
{
    ...
    MArrayDataHandle transformsHandle =
        block.inputArrayValue(matrix);
    int numTransforms =
        transformsHandle.elementCount();
    if(numTransforms == 0)
        return MS::kSuccess;
    int precomp_event = MProfiler::eventBegin(
            corSkinCluster::_profileCategory,
            MProfiler::kColorG_L1, "corSkinCluster: precomp");
    // insert precomp test here
    MDataHandle validHandle = block.inputValue(
            cor_valid, &returnStatus);
   ...
   //         validHandle.setBool(true);
    ...
    MObject cor_arData = cor_arHandle.data();
    MFnPointArrayData cor_arFn(cor_arData,
        &returnStatus);
    if(returnStatus != MS::kSuccess)
        {
    ...
    MArrayDataHandle weightListHandle =
        block.inputArrayValue(weightList);
    ...
    //
    int skin_event = MProfiler::eventBegin(
            corSkinCluster::_profileCategory,
            MProfiler::kColorG_L1,
            "corSkinCluster: deform loop");
    for(int i = 0; !iter.isDone(); i++)
        {
            MPoint v_i = iter.position();
            MPoint vprime_i;
            MArrayDataHandle weightsHandle =
                weightListHandle.inputValue().child(weights);
            ...
            //LBS Matrix
            MMatrix R_t_prime =
                MMatrix::identity;  // init to identity
            for(int j = 0; j < numTransforms; j++)
                {
                    MTransformationMatrix tm(transforms[j]);
                    MTransformationMatrix temp = tm.asRotateMatrix();
                    temp.setTranslation(tm.getTranslation(
                            MSpace::kWorld, NULL),
                        MSpace::kWorld);  // switched from kWorld
                    MStatus stat;
                    stat = weightsHandle.jumpToElement(j);
                    if(stat == MStatus::kSuccess)
                        w_j = weightsHandle.inputValue().asDouble();
            ...
            MVector t = (cor_PA[i] *
                    R_t_prime_tm.asRotateMatrix()) +
                R_t_prime_tm.getTranslation(MSpace::kWorld,
                    NULL) - (cor_PA[i] * R);  // switch from kWorld
            vprime_i = v_i * R + t;
...
MString corSkinGPUDeformer::plugin_path =
    MString("");
MString corSkinGPUDeformer::openCL_source_file =
    "corSkinDef.cl";

MString corSkinGPUDeformer::kName("corSkinGPUOverride");
...
// static
bool corSkinGPUDeformer::validateNodeInGraph(
    MDataBlock& block,
...
// static
bool corSkinGPUDeformer::validateNodeValues(
    MDataBlock& block,
    const MEvaluationNode& evaluationNode,
    const MPlug& plug, MStringArray* messages)
{
    ...
    if(!validData.asBool())
        {
            MOpenCLInfo::appendMessage(messages,
                "corSKin %s not supported by deformer evaluator because precomputation is invalid.",
                fnNode.name().asChar());
            return false;
        }
    return true;
    ...
    MAutoCLEvent& outputEvent) // the event a downstream deformer will wait for before reading from output buffer
{
	...
	//bypass if invalid config
	MStatus status;
	//MFnDependencyNode fnDG(node, &status);
	MDataHandle hValid = block.inputValue(corSkinCluster::cor_valid, &status);
	if (status.error()) {
	...
	//	return MPxGPUDeformer::kDeformerPassThrough;
	//}
	//MPlug pValid = fnDG.findPlug(corSkinCluster::cor_valid, &status);
	//if (status.error()) {
	//	return MPxGPUDeformer::kDeformerPassThrough;
	//}
	//bool bValid;
	//status = pValid.getValue(bValid);
	//if (status.error() || !bValid) {
	//	return MPxGPUDeformer::kDeformerPassThrough;
	//}
	// end bypass invalid config
    ...
    if(!fKernel.get())
        {
            ...
            MString filepath = corSkinGPUDeformer::plugin_path
                + MString("/") +
                corSkinGPUDeformer::openCL_source_file;
            MString openCLKernelFile(filepath);
            MString openCLKernelName("corSkinDef");
            fKernel = MOpenCLInfo::getOpenCLKernel(
                    openCLKernelFile, openCLKernelName);
            if(fKernel.isNull()) return
                    MPxGPUDeformer::kDeformerFailure;
        }
    cl_int err = CL_SUCCESS;
    // Set all of our kernel parameters.  Input buffer and output buffer may be changing every frame
    // so always set them.
    unsigned int parameterId = 0;
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem),
            (void*)outputBuffer.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem),
            (void*)inputBuffer.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem),
            (void*)fCLWeights.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem), (void*)fTM.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem), (void*)fQ.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_mem), (void*)fCoR.getReadOnlyRef());
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_uint), (void*)&fNumElements);
    MOpenCLInfo::checkCLErrorStatus(err);
    err = clSetKernelArg(fKernel.get(), parameterId++,
            sizeof(cl_uint), (void*)&fNumTransforms);
    MOpenCLInfo::checkCLErrorStatus(err);
    ...
    size_t globalWorkSize = (localWorkSize -
            fNumElements % localWorkSize) +
        fNumElements; // global work size must be a multiple of localWorkSize
...
void corSkinGPUDeformer::extractWeightArray(
    MDataBlock& block,
    const MEvaluationNode& evaluationNode,
    const MPlug& plug)
{
    ...
    // corSkinCluster node.
    if((!fCLWeights.isNull()
            && !evaluationNode.dirtyPlugExists(
                corSkinCluster::weightList, &status)) || !status)
        return;
    ...
    MArrayDataHandle transformsHandle =
        block.inputArrayValue(corSkinCluster::matrix);
    int numTransforms =
        transformsHandle.elementCount();
    if(numTransforms == 0) return;
    MStatus stat;
    MArrayDataHandle weightListHandle =
        block.inputArrayValue(corSkinCluster::weightList,
            &stat);
    if(stat.error()) return;
    for(unsigned int i = 0; i < fNumElements; i++)
        {
            MArrayDataHandle weightsHandle =
                weightListHandle.inputValue().child(
                    corSkinCluster::weights);
            for(int j = 0; j < numTransforms; j ++)
                {
                    stat = weightsHandle.jumpToElement(j);
                    if(stat.error())
                        ...
                        {
                            double asDbl =
                                weightsHandle.inputValue().asDouble();
                            float asFlt =
                                weightsHandle.inputValue().asFloat();
    ...
    if(!fCLWeights.get())
        {
            ...
            fCLWeights.attach(clCreateBuffer(
                    ...
                    fNumElements * numTransforms * sizeof(float),
                    (void*)&temp[0], &err));
        }
    else
        {
            ...
            err = clEnqueueWriteBuffer(
                    MOpenCLInfo::getMayaDefaultOpenCLCommandQueue(),
                    fCLWeights.get(), CL_TRUE, 0,
                    fNumElements * numTransforms * sizeof(float),
                    (void*)&temp[0], 0, NULL, NULL);
        }
}

MStatus corSkinGPUDeformer::extractCoR(
    MDataBlock& block,
    const MEvaluationNode& evaluationNode,
    const MPlug& plug)
{
    ...
    if((fCoR.get()
            && !evaluationNode.dirtyPlugExists(
                corSkinCluster::weightList, &stat)) || !stat)
        return stat;
    // if they're not, pull them off
    MDataHandle cor_arHandle = block.inputValue(
            corSkinCluster::cor_ar, &stat);
    if(stat != MS::kSuccess)
        return stat;
    MObject cor_arData = cor_arHandle.data();
    MFnPointArrayData cor_arFn(cor_arData, &stat);
    ...
    if(!fCoR.get())
        {
            ...
            fCoR.attach(clCreateBuffer(
                    MOpenCLInfo::getOpenCLContext(),
                    CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                    fNumElements * 4 * sizeof(float), (void*)&temp[0],
                    &err));
        }
    else
        {
...
void corSkinGPUDeformer::extractTMnQ(
    MDataBlock& block,
    const MEvaluationNode& evaluationNode,
    const MPlug& plug)
{
    ...
    if((fTM.get()
            && !evaluationNode.dirtyPlugExists(
                corSkinCluster::matrix, &status)) || !status)
        return;
    MArrayDataHandle transformsHandle =
        block.inputArrayValue(corSkinCluster::matrix);
    int numTransforms =
    ...
    MArrayDataHandle bindHandle =
        block.inputArrayValue(
            corSkinCluster::bindPreMatrix);
    if(bindHandle.elementCount() > 0)
        {
            for(int i = 0; i < numTransforms; ++i)
                {
    ...
    for(int i = 0; i < numTransforms; i++)
        {
            ...
            temp_trans = temp_tm.getTranslation(
                    MSpace::kWorld);
            temp = temp_tm.asRotateMatrix();
            temp_tm = MTransformationMatrix(temp);
            temp_tm.setTranslation(temp_trans,
                MSpace::kWorld);
    ...
    for(int i = 0; i < numTransforms; i++)
        {
            for(int r = 0; r < 4; r++)
                {
                    for(int c = 0; c < 4; c++)
                        conv_transforms.push_back((float)
                            transforms_transpose[i](r, c));
                }
        }
    // now we prep that group for openCL
    cl_int err = CL_SUCCESS;
    if(!fTM.get())
        fTM.attach(clCreateBuffer(
                MOpenCLInfo::getOpenCLContext(),
                CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                conv_transforms.size() * sizeof(float),
                (void*)&conv_transforms[0], &err));
    else
        {
            // I use a blocking write here, non-blocking could be faster...  need to manage the lifetime of temp, and have the kernel wait until the write finishes before running
            err = clEnqueueWriteBuffer(
                    ...
                    fTM.get(), CL_TRUE, 0,
                    conv_transforms.size() * sizeof(float),
                    (void*)&conv_transforms[0], 0, NULL, NULL);
        }
    ...
    err = CL_SUCCESS;
    if(!fQ.get())
        fQ.attach(clCreateBuffer(
                MOpenCLInfo::getOpenCLContext(),
                CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY,
                conv_q.size() * sizeof(float), (void*)&conv_q[0],
                &err));
    else
        {
            // I use a blocking write here, non-blocking could be faster...  need to manage the lifetime of temp, and have the kernel wait until the write finishes before running
...
//{
//    MStatus result;
//    MFnPlugin plugin(obj, "Benjamin Slack", "0.1",
//        "Any");
...
//    corSkinGPUDeformer::plugin_path =
//        plugin.loadPath();
//    return result;
//}
